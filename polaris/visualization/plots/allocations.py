...
class AllocationPlotter(MultiAgentPlotter):
    ...
    def plot_allocations_over_time(self, metrics: Dict, output_dir: Path):
        ...
        for i, (agent_id, agent_allocations) in enumerate(allocations.items()):
            ...
            ax.plot(timesteps, allocations_np, label=label, 
        ...
        if 'theoretical_bounds' in metrics:
            ...
            if 'mpe_neutral' in bounds:
                ax.axhline(y=bounds['mpe_neutral'], color='k', linestyle='--', 
                          alpha=0.7, label='MPE (neutral)')
            if 'mpe_good_state' in bounds:
                ax.axhline(y=bounds['mpe_good_state'], color='g', linestyle='--', 
                          alpha=0.7, label='MPE (good state)')
            if 'mpe_bad_state' in bounds:
                ax.axhline(y=bounds['mpe_bad_state'], color='r', linestyle='--', 
                          alpha=0.7, label='MPE (bad state)')
    ...
    def plot_kl_divergence(self, metrics: Dict, output_dir: Path):
        ...
        for i, (agent_id, agent_kl) in enumerate(kl_divergences.items()):
            ...
            ax.plot(timesteps[:len(smoothed_kl)], smoothed_kl, 
                   label=f'Agent {agent_id}', 
                   color=agent_color, linewidth=1.5)
    ...
    def plot_policy_cutoff_vs_belief(self, metrics: Dict, output_dir: Path):
        ...
        for i, agent_id in enumerate(metrics['allocations'].keys()):
            if (agent_id not in metrics['agent_beliefs'] or 
                agent_id not in metrics['allocations']):
                continue
            ...
            ax.scatter(beliefs, allocations, 
                      label=f'Agent {int(agent_id)}', 
                      alpha=0.6, s=15, color=agent_color)
            ...
            if len(beliefs) >= 2:
                ...
                if np.std(x) > 1e-6:
                    ...
                    ax.plot(reg_x, reg_y, linestyle='--', linewidth=2, 

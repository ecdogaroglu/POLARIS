...
class LearningCurvePlotter(MultiAgentPlotter):
    ...
    def plot(self, metrics: Dict, env, args, output_dir: Path, episodic_metrics=None):
        ...
        if not self.validate_metrics(metrics):
            self.create_empty_plot(output_dir, "learning_curves.png", "No learning data available")
            return
        
        # Skip incorrect probability plots for strategic experimentation (Keller-Rady) experiments
        if hasattr(env, 'safe_payoff'):
            print("  📈 Skipping incorrect probability plots for strategic experimentation environment")
            return
    ...
    def _plot_episodic_learning_curves(self, episodic_metrics: Dict, args, output_dir: Path):
    ...
    def _plot_standard_learning_curves(self, metrics: Dict, args, output_dir: Path):
        ...
        incorrect_probs = self._process_incorrect_probabilities(metrics, metrics.get('num_agents', 2))
    ...
    def plot_mean_incorrect_action_probabilities_with_ci(
        ...
        # Plot mean line
        if log_scale:
            ax.semilogy(time_steps, mean_probs, 
                       label=f"Mean ({num_episodes} episodes)",
                       color=colors[0], linewidth=2)
        else:
            ax.plot(time_steps, mean_probs,
                   label=f"Mean ({num_episodes} episodes)", 
                   color=colors[0], linewidth=2)
        
        # Plot confidence interval
        ax.fill_between(time_steps, 
                       mean_probs - ci, mean_probs + ci,
    ...
    def plot_incorrect_action_probabilities(
        ...
        for i, (agent_id, probs) in enumerate(incorrect_probs.items()):
            ...
            # Plot the line
            if log_scale:
                ax.semilogy(time_steps, probs, label=label, 
                           color=agent_color, linewidth=1.5)
            else:
                ax.plot(time_steps, probs, label=label,
                       color=agent_color, linewidth=1.5)
    ...
    def _process_incorrect_probabilities(self, metrics: Dict, num_agents: int) -> Dict[int, List[float]]:
        """

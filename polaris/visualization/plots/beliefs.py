...
class BeliefPlotter(MultiAgentPlotter):
    ...
    def plot(self, metrics: Dict[str, Any], env, args, output_dir: Path):
        ...
        if not hasattr(env, 'safe_payoff') and 'belief_distributions' in metrics and len(agent_ids) > 1:
    ...
    def plot_belief_distributions(self, metrics: Dict, agent_id: int, num_states: int, output_dir: Path):
        """
        ...
        if belief_values.ndim == 1:
            ...
            ax.plot(time_steps, belief_values, 
                   label=f'State 1 (Good)', 
                   color=colors[1 % num_states], linewidth=2)
            # Plot complement for state 0
            ax.plot(time_steps, 1 - belief_values, 
                   label=f'State 0 (Bad)', 
            ...
            for state in range(min(num_states, belief_values.shape[1])):
                ax.plot(time_steps, belief_values[:, state], 
                       label=f'State {state}', 
            ...
            for state in range(belief_values.shape[1]):
                ax.plot(time_steps, belief_values[:, state], 
                       ...
                       color=colors[state % num_states], linewidth=2)
    ...
    def plot_good_belief_over_time(self, metrics: Dict, output_dir: Path):
        ...
        for i, (agent_id, beliefs) in enumerate(metrics['agent_beliefs'].items()):
            ...
            if good_beliefs:  # Only plot if we have valid beliefs
                ...
                ax.plot(time_steps, good_beliefs, 
    ...
    def plot_social_learning_beliefs(self, metrics: Dict, env, output_dir: Path):
        ...
        for i, (agent_id, belief_distributions) in enumerate(metrics['belief_distributions'].items()):
            if not belief_distributions:
                continue
            ...
            for t, belief_dist in enumerate(belief_distributions):
                try:
                    ...
                    if not np.isnan(wrong_state_belief) and 0 <= wrong_state_belief <= 1:
            ...
            if belief_in_wrong_state:
                has_data = True
                agent_color = self.get_agent_color(i, colors)
                ax.plot(time_steps, belief_in_wrong_state, 
                       label=f'Agent {agent_id}', 
        ...
        # Add reference line at 0.5
        ax.axhline(y=0.5, color='gray', linestyle='--', alpha=0.5, label='Random Belief')
        
        # Formatting
        ax.set_title(f'Agents\' Belief in Wrong State (State {wrong_state}) Over Time\nTrue State: {true_state}')
        ax.set_xlabel('Time Steps')

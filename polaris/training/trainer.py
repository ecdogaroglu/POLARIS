...
class Trainer:
    ...
    def run_agents(self, training=True, model_path=None):
        ...
        load_agent_models(self.agents, model_path, self.env.num_agents, training=training)
    ...
    def _run_training(self, theoretical_bounds):
        ...
        print(f"Running {self.args.num_episodes} training episode(s) with {self.args.horizon} steps per episode")
    ...
    def _run_evaluation(self, theoretical_bounds):
        ...
        print(f"Running evaluation for {self.args.num_episodes} episode(s) with {self.args.horizon} steps per episode")
        ...
        serializable_metrics = prepare_serializable_metrics(
            combined_metrics, learning_rates, theoretical_bounds, self.args.horizon, training=False
        )
        ...
        save_metrics_to_file(evaluation_serializable_metrics, self.output_dir, training=False, filename='detailed_evaluation_results.json')
    ...
    def _process_training_results(self, episodic_metrics, theoretical_bounds):
        ...
        combined_metrics = flatten_episodic_metrics(episodic_metrics, self.env.num_agents)
        ...
        serializable_metrics = prepare_serializable_metrics(
            combined_metrics, learning_rates, theoretical_bounds, self.args.horizon, training=True
        )
        ...
        save_metrics_to_file(episodic_serializable_metrics, self.output_dir, training=True, filename='episodic_metrics.json')
    ...
    def _run_training_simulation(self):
        ...
        for step in steps_iterator:
            ...
            next_observations, rewards, done, info = self.env.step(actions, action_probs)
            ...
            if hasattr(self.args, 'continuous_actions') and self.args.continuous_actions and hasattr(self.env, 'safe_payoff'):
            ...
            update_progress_display(steps_iterator, info, total_rewards, step, training=True)
    ...
    def _print_environment_info(self):
        ...
        if hasattr(self.env, 'safe_payoff'):
            ...
            if self.env.true_state == 0:
                print(f"True state is bad. Drift rate: {self.env.drift_rates[self.env.true_state]} "
                      f"Jump rate: {self.env.jump_rates[self.env.true_state]} "
                      f"Jump size: {self.env.jump_sizes[self.env.true_state]}")
            else:
                print(f"True state is good. Drift rate: {self.env.drift_rates[self.env.true_state]} "
                      f"Jump rate: {self.env.jump_rates[self.env.true_state]} "
                      f"Jump size: {self.env.jump_sizes[self.env.true_state]}")
        else:
    ...
    def _update_agent_states_training(self, observations, next_observations, actions, rewards, step):
        """Update agent states and store transitions in replay buffer during training."""
        
        # Check if we're using continuous actions
        continuous_actions = hasattr(self.args, 'continuous_actions') and self.args.continuous_actions
        ...
        for agent_id, agent in self.agents.items():
            ...
            self.metrics['belief_distributions'][agent_id].append(belief_distribution.detach().cpu().numpy())
            ...
            if agent_id in self.replay_buffers:
                ...
                if len(self.replay_buffers[agent_id]) > self.args.batch_size and step % self.args.update_interval == 0:
    ...
    def _initialize_agents(self, obs_dim):
        ...
        print(f"Initializing {self.env.num_agents} agents{' for evaluation' if self.args.eval_only else ''}...")
        
        # Log if using GNN
        if self.args.use_gnn:
            print(f"Using Graph Neural Network with {self.args.gnn_layers} layers, {self.args.attn_heads} attention heads, and temporal window of {self.args.temporal_window}")
        else:
            print("Using traditional encoder-decoder inference module")
        
        # Log if excluding final layers from SI
        if hasattr(self.args, 'si_exclude_final_layers') and self.args.si_exclude_final_layers and hasattr(self.args, 'use_si') and self.args.use_si:
        ...
        for agent_id in range(self.env.num_agents):
            ...
            agent = POLARISAgent(
                ...
                si_importance=self.args.si_importance if hasattr(self.args, 'si_importance') else 100.0,
                si_damping=self.args.si_damping if hasattr(self.args, 'si_damping') else 0.1,
                si_exclude_final_layers=self.args.si_exclude_final_layers if hasattr(self.args, 'si_exclude_final_layers') else False,
                continuous_actions=self.args.continuous_actions if hasattr(self.args, 'continuous_actions') else False
            )

...
class Evaluator:
    ...
    def evaluate(self, num_episodes: int = None, num_steps: int = None, 
        ...
        eval_episodes = num_episodes if num_episodes is not None else getattr(self.args, 'num_episodes', 1)
        eval_steps = num_steps if num_steps is not None else getattr(self.args, 'horizon', 1000)
        
        print(f"Starting evaluation for {eval_episodes} episode(s) with {eval_steps} steps each")
        ...
        for episode in range(eval_episodes):
            ...
            episode_seed = self.args.seed + episode + 1000  # Offset to avoid training seed overlap
        ...
        evaluation_summary = self._calculate_evaluation_summary(episodic_metrics, learning_rates)
    ...
    def _evaluate_single_episode(self, num_steps: int) -> Dict[str, Any]:
        ...
        for step in steps_iterator:
            ...
            update_progress_display(steps_iterator, info, total_rewards, step, training=False)
        ...
        self.metrics['total_rewards'] = {i: total_rewards[i] for i in range(self.env.num_agents)}
        self.metrics['final_observations'] = observations
        
        return self.metrics
    
    def _update_agent_states_for_evaluation(self, observations, next_observations, 
                                           actions, rewards, step):
                    ...
                    if hasattr(agent, 'current_belief_distribution') and agent.current_belief_distribution is not None:
                        if torch.isnan(agent.current_belief_distribution).any():
                            ...
                            if agent.current_belief_distribution.size(1) == 1:
                                ...
                                belief_weights = belief_weights / (agent.current_belief_distribution.shape[-1] - 1)
    ...
    def _aggregate_episode_results(self, episodic_metrics: Dict) -> Dict[str, Any]:
        ...
        for key, value in first_episode.items():
            if key == 'episode_time':
                ...
                times = [ep.get('episode_time', 0) for ep in episodic_metrics['episodes']]
                aggregated[key] = np.mean(times)
            elif key == 'total_rewards':
                # Average total rewards per agent
                all_rewards = [ep.get('total_rewards', {}) for ep in episodic_metrics['episodes']]
                if all_rewards and all_rewards[0]:
                    aggregated[key] = {}
                    for agent_id in all_rewards[0].keys():
                        rewards = [ep_rewards.get(agent_id, 0) for ep_rewards in all_rewards]
    ...
    def _calculate_evaluation_summary(self, episodic_metrics: Dict, 
                                    learning_rates: Dict) -> Dict[str, Any]:
        ...
        if episodic_metrics['episodes'] and 'total_rewards' in episodic_metrics['episodes'][0]:
            reward_stats = {}
            for agent_id in range(self.env.num_agents):
                rewards = [ep['total_rewards'].get(agent_id, 0) 
                          for ep in episodic_metrics['episodes'] 
        ...
        if (episodic_metrics['episodes'] and 
            any('true_states' in ep for ep in episodic_metrics['episodes']) and
            any('agent_actions' in ep for ep in episodic_metrics['episodes'])):
            ...
            for agent_id in range(self.env.num_agents):
                ...
                for ep in episodic_metrics['episodes']:
                    if 'true_states' in ep and 'agent_actions' in ep:
                        ...
                        for i, (true_state, action) in enumerate(zip(true_states, agent_actions)):
    ...
    def quick_evaluate(self, num_steps: int = 100) -> Dict[str, float]:
        ...
        results = {
            ...
            'total_rewards': {i: float(total_rewards[i]) for i in range(self.env.num_agents)},
        }
        
        if hasattr(self.env, 'true_state'):
            results['action_accuracy'] = {
                i: float(correct_actions[i] / num_steps) for i in range(self.env.num_agents)
            }
